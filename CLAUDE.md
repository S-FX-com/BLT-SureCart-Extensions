...
- **Reports** — generates a **Fulfillment Report** CSV: every order in a date range (optionally narrowed to selected products) collapsed into one row per customer with a quantity column per product variant, plus a TOTALS row for placing the manufacturing order. Read-only — it never writes to SureCart and never touches checkout. Three things to know before changing it, all detailed in `tasks/00-discovery.md` §H: (1) **`GET /v1/orders` has no date filter and no sort parameter**, so the date range is applied in PHP against `order.created_at` and the job cannot early-exit on a date boundary — don't "optimize" that into a break, page order is undefined and you'd silently drop orders. (2) **`product_ids[]` filters orders, not the line items inside them**, so the selection is applied a second time per line item in `FulfillmentMatrix` — remove that and a single-product report starts totalling whatever else those customers bought in the same order. (3) **The quantity basis is deliberately mode-dependent** — ordered `quantity` for a manufacturing count, `quantity - fulfilled_quantity` for the outstanding-work presets. Collapsing those two into one number re-ships units on partially fulfilled orders.
...
10. **Never use `isset()`, `empty()`, or `property_exists()` on a SureCart response object — use `Support\Obj`.** Verified against SureCart 4.6.2's shipped source by the backtest in `tests/backtest/` (which loads SureCart's real model code): a list query's `paginate()` returns `SureCart\Models\Collection`, and *that* class defines `__get()` with **no `__isset()`** and implements no `ArrayAccess` — so `isset( $collection->data )` is `false` while `$collection->data` holds every record. That single class is how 0.3.0 shipped a "successful" fulfillment report containing zero orders against a store with 96 shirts sold, and why the product picker stayed empty. The `Model` class itself *does* implement `__isset()` (and `ArrayAccess`), and nested list envelopes like `checkout->line_items` hydrate as plain `stdClass` — so `isset()` *happens* to work on those today, which is precisely what makes the trap easy to reintroduce: it works everywhere except the one wrapper every list query passes through. `Obj::get()/str()/int()/obj()/items()` handle arrays, `stdClass`, `ArrayAccess`, and magic-accessor objects uniformly (`Obj::items()` also unwraps the `{ data: [] }` envelope), so use it for every read and don't memorize which class lies. Same for list parsing — `Api\SureCartGateway::normalize_page()` accepts every envelope shape and reports the runtime shape it saw. Run `tests/backtest/run.sh` after touching any of this: it exercises the full job pipeline (query → real SureCart models → CSV) against fixture data with an independent oracle, and asserts the exact wire format (`status[]=`, `expand[]=`, `limit`, pagination, auth header).

## Admin page URLs

These screens are `admin.php` query-string routes, not files: `wp-admin/admin.php?page=blt-sce-reports`. `wp-admin/blt-sce-reports` is not a valid URL and resolves to the *theme's* 404, which looks nothing like a WordPress error. Always build links with `add_query_arg( array( 'page' => … ), admin_url( 'admin.php' ) )`, as the existing screens do. Registered slugs: `blt-sce-modules`, `blt-sce-settings`, `blt-sce-reports`, `blt-sce-shipments`, `blt-sce-review-queue`, `blt-sce-offers`, `blt-sce-offer-settings`, `blt-sce-price-restrictions`.
